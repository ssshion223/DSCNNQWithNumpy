...
class model:
    ...
    def depthwiseconv2d(self, input, weight_q, stride, scale_in, scale_out, scale_weight, bias):
        ...
        # steps and domain transform: input(uint8), convolution(int32), add bias(int32), quantization(uint8), activation(uint8)
    ...
    def conv2d(self, input, weight_q, scale_in, scale_out, scale_weight, bias): # 1x1
        ...
        for i in range(64):
            for y in range(25):
                for x in range(5):
                    output[0, y, x, i] = np.sum((kernel[i, 0, 0, :].astype(np.int32)-128) * (input[0, y, x, :].astype(np.int32)-128))
        output = self.add_bias(output, bias)
        output = self.quantization(output, scale_in, scale_weight, scale_out)
        return self.Relu(output)
    ...
    def quantization(self, input, scale_in, scale_weight, scale_out, zero=128):
        real = input * scale_in * scale_weight / scale_out
    ...
    def prediction(self, mfcc_in):  # 49x10
        ...
        # input_std = (mfcc_in - 128) / 0.51619
        # input_q = np.round(mfcc_in / 1.93727) + 128
        ###downsample
        downsample = self.depthwiseconv2d(mfcc_in, "weights\DS-CNN_dw0.npy", stride=2, scale_in=1.937271237373352, scale_out=0.10932768881320953,
                                          scale_weight=0.0007880174671299756, bias="weights\DS-CNN_dw0_Fold_bias.npy")
        ...
        layer1_dw = self.depthwiseconv2d(downsample, "weights\DS-CNN_dw1.npy", stride=1, scale_in=0.10932768881320953, scale_out=0.124430350959301,
                                         scale_weight=0.02164473570883274, bias="weights\DS-CNN_dw1_Fold_bias.npy")
        layer1_pw = self.conv2d(layer1_dw, "weights\DS-CNN_pw1.npy", scale_in=0.124430350959301, scale_out=0.10584013164043427,
                                scale_weight=0.005275495816022158, bias="weights\DS-CNN_pw1_Fold_bias.npy")
        ...
        layer2_dw = self.depthwiseconv2d(layer1_pw, "weights\DS-CNN_dw2.npy", stride=1, scale_in=0.10584013164043427, scale_out=0.1091616228222847,
                                         scale_weight=0.013021008111536503, bias="weights\DS-CNN_dw2_Fold_bias.npy")
        layer2_pw = self.conv2d(layer2_dw, "weights\DS-CNN_pw2.npy", scale_in=0.1091616228222847, scale_out=0.07970251142978668,
                                scale_weight=0.005137702450156212, bias="weights\DS-CNN_pw2_Fold_bias.npy")
        ###layer333
        layer3_dw = self.depthwiseconv2d(layer2_pw, "weights\DS-CNN_dw3.npy", stride=1, scale_in=0.07970251142978668, scale_out=0.09610754996538162,
                                         scale_weight=0.012005737982690334, bias="weights\DS-CNN_dw3_Fold_bias.npy")
        layer3_pw = self.conv2d(layer3_dw, "weights\DS-CNN_pw3.npy", scale_in=0.09610754996538162, scale_out=0.07651054859161377,
                                scale_weight=0.005375369917601347, bias="weights\DS-CNN_pw3_Fold_bias.npy")
        ###layer444
        layer4_dw = self.depthwiseconv2d(layer3_pw, "weights\DS-CNN_dw4.npy", stride=1, scale_in=0.07651054859161377, scale_out=0.11575359851121902,
                                         scale_weight=0.009517693892121315, bias="weights\DS-CNN_dw4_Fold_bias.npy")

        layer4_pw = self.conv2d(layer4_dw, "weights\DS-CNN_pw4.npy", scale_in=0.11575359851121902, scale_out=0.08714716881513596,
                                scale_weight=0.007161697372794151, bias="weights\DS-CNN_pw4_Fold_bias.npy")
        print(layer4_pw.shape)
        ###
        layer_avgpool = self.globalaveragepool2d(layer4_pw)
        print(layer_avgpool.shape)
